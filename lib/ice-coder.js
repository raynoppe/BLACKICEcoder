...
// ==============
	...
	// Define settings
	filesW:			200,		// Width of files pane
	minFilesW:		25,		// Min width of files pane
	maxFilesW:		200,		// Max width of files pane
...
// ==============
	...
	setLayout: function(dontSetEditor) {
		...
		// this.header.style.width = winW + "px";
		// this.tabsBar.style.width = winW - 50 + "px";
		// this.findBar.style.width = winW + "px";
...
// ==============
	...
	setMinimap: function() {
		...
		// 		top.ICEcoder.miniMapBoxHeight+'px; background: rgba(0,198,255,0.1); z-index: 1; cursor: pointer" id="miniMapBox"></div>';
	...
	setMinimapLayout: function(thisCM,cMinstance) {
		...
		// 		top.ICEcoder.miniMapBoxTop = (percThru*parseInt(get('miniMapContainer').getBoundingClientRect().height,10)) - (percThru*top.ICEcoder.miniMapBoxHeight);
...
// ==============
	...
	newFile: function() {
		...
		shortURL = ICEcoder.LastFolder;
		filetosave = '';
		console.log('newFile', shortURL);
		rawUrl = shortURL;
		if(shortURL !== '/') { shortURL += '/'; }
		var nfp = prompt('Please enter a file name to save in '+shortURL,'untitled.js');
		if (nfp != null) {
			console.log('nfp', nfp);
			saveURL = 'lib/save.php?action=create&csrf='+top.ICEcoder.csrf;
			filetosave = shortURL + nfp;
			...
			xhr.onreadystatechange=function() {
				if (xhr.readyState==4 && xhr.status==200) {
					...
					if (statusObj.status === 'created') {
						top.ICEcoder.updateFileManagerList('add',rawUrl,nfp,null,null,null,'file');
						// updateFileManagerList: function(action,location,file,perms,oldName,uploaded,fileOrFolder)
						// 
					} else if(statusObj.status === 'exists') {
						alert('Sorry but I can not and will not overwrite a file that exist. Call it a moral issue.');
					} else {
						alert('An error has occured. Please call someone.');
					}
				}
			};
			...
			xhr.send('timeStart=0&file=' + filetosave + '&contents=');
		}
	...
	newFolder: function() {
		...
		console.log('ICEcoder.LastFolder', ICEcoder.LastFolder);
		// shortURL = top.ICEcoder.selectedFiles[top.ICEcoder.selectedFiles.length-1].replace(/\|/g,"/");
		shortURL = ICEcoder.LastFolder;
	...
	openFile: function(fileLink) {
		console.log('openFile: fileLink', fileLink);
		var flSplit, line = 1, shortURL, canOpenFile;
		...
		if (top.ICEcoder.thisFileFolderLink != "/[NEW]" && top.ICEcoder.isOpen(top.ICEcoder.thisFileFolderLink)!==false) {
			...
			if (canOpenFile) {
				...
				if (shortURL!="/[NEW]") {
					...
					// console.log('top.ICEcoder.csrf', top.ICEcoder.csrf);
					// console.log('line', line);
					// console.log('send', "lib/file-control.php?action=load&file="+top.ICEcoder.thisFileFolderLink.replace(/\+/g,"%2B")+"&csrf="+top.ICEcoder.csrf+"&lineNumber="+line);
	...
	saveFile: function(saveAs) {
		...
		console.log('saveFile', ICEcoder.selectedTab);
	...
	updateFileManagerList: function(action,location,file,perms,oldName,uploaded,fileOrFolder) {
		...
		console.log('updateFileManagerList');	
...
// ==============
					...
					if (xhr.readyState==4) {
						...
						if (xhr.status==200) {
							...
							if (statusObj.status.error) {
								...
								console.log('statusObj.action.doNext', statusObj.action.doNext);
								eval(statusObj.action.doNext);
							}
						// Some other response? Display a message about that
						} else {
	...
	settingsScreen: function(hide) {
		if (!hide) {
			top.get('mediaContainer').innerHTML = '<iframe src="lib/settings-screen.php" class="whiteGlow" style="width: 80%; height: 500px"></iframe>';
		}
...
// ==============
	...
	switchTab: function(newTab,noFocus) {
		...
		if ( newTab !== 'ignore' ) {
			var tabName = newTab.replace('tab', '');
			ICEcoder.selectedTab = tabName;
			console.log('tabName', tabName);
			top.ICEcoder.redoTabHighlight(tabName);
		}
	...
	newTab: function(alsoSave) {
		...
		// top.ICEcoder.createNewTab(true);
	...
	createNewTab: function(isNew = false) {
		var closeTabLink, fileName;
		console.log('isNew', isNew);
		// Push new file into array
		var newTabName = top.ICEcoder.shortURL;
		// see if the tab is open
		var tabtrue = ICEcoder.openFiles.indexOf(newTabName);
		
		console.log('tabtrue', tabtrue);
		if ( tabtrue != -1 ) {

		} else {
			var addp = top.ICEcoder.openFiles.push(newTabName);

			console.log('top.ICEcoder.openFiles', top.ICEcoder.openFiles, addp);

			// Setup a new tab
			var div = document.createElement('div');
			div.id = 'tab' + top.ICEcoder.shortURL;
			div.className = 'tab';
			closeTabLink = '<a nohref onClick="top.ICEcoder.closeTab(this.parentNode.id)"><img src="images/nav-close.gif" class="closeTab"></a>';
			fileName = top.ICEcoder.openFiles[top.ICEcoder.openFiles.length-1];
			div.innerHTML = closeTabLink + top.ICEcoder.shortURL.slice(top.ICEcoder.shortURL.lastIndexOf("/")).replace(/\//,"");
			divinto = document.getElementById('divcontainer');
			divinto.appendChild(div);

			// create new content area
			var cdiv = document.createElement('div');
			cdiv.className = 'rightbarContentInner';
			cdiv.id = 'content' + top.ICEcoder.shortURL;
			cdiv.innerHTML = '<iframe name="contentFrame" id="content" src="editor.php?isNew='+ isNew +'&getfile='+ top.ICEcoder.shortURL +'&csrf='+ top.ICEcoder.csrf +'" class="code" scrolling="no"></iframe>';
			divinto2 = document.getElementById('rightbarContent');
			divinto2.appendChild(cdiv);
			...
			top.ICEcoder.redoTabHighlight(newTabName);
	...
	redoTabHighlight: function(selectedTab) {
		var tColor, fileLink;
		console.log('redoTabHighlight', selectedTab);
		for ( tab of ICEcoder.openFiles ) {
			var actTab = document.getElementById('tab'+tab);
			console.log('actTab', tab);
			
			var actCon = document.getElementById('content'+tab);
			if ( tab === selectedTab ) {
				actTab.style.backgroundColor = 'red';
				// actTab.removeAttribute("onclick");
				actTab.setAttribute("onclick","top.ICEcoder.switchTab('ignore');");
				actCon.style.display = 'flex';
			} else {
				actTab.style.backgroundColor = '#333';
				actTab.setAttribute("onclick","top.ICEcoder.switchTab(this.id);");
				actCon.style.display = 'none';
			}
		}
	...
	closeTab: function(closeTabNum, dontSetPV, dontAsk) {
		...
		tabName = closeTabNum.replace('tab', '');
		var sf = ICEcoder.openFiles.indexOf(tabName);

		ICEcoder.openFiles.splice(sf, 1);
		// If we haven't specified, close current tab
		if (!closeTabNum) {closeTabNum = top.ICEcoder.selectedTab};
		var elem = document.getElementById(closeTabNum);
		elem.parentNode.removeChild(elem);

		var contentname = closeTabNum.replace('tab', 'content');
		var elem2 = document.getElementById(contentname);
...
// ==============
	...
	interceptKeys: function(area, evt) {
		...
		if (top.ICEcoder.snakePlaying) {
			if (key==37) {top.ICEcoder.snakeDir = 'left'}
			if (key==39) {top.ICEcoder.snakeDir = 'right'}
			if (key==38) {top.ICEcoder.snakeDir = 'up'}
			if (key==40) {top.ICEcoder.snakeDir = 'down'}
			return false;
		}
	...
	// Snart snake
	startSnake: function() {
		top.ICEcoder.snakePlaying = true;
		top.ICEcoder.showHide('show',top.get('blackMask'));
		top.get('mediaContainer').innerHTML = '<span style="font-size: 14px">Let\'s play<br><img src="images/snake.png" alt="snake"><br><br><br>Use arrow keys to eat your code<br><br>(it returns afterwards of course) :-)</span>';
		setTimeout(function() {
			top.ICEcoder.showHide('hide',top.get('blackMask'));
			top.get('mediaContainer').innerHTML = '';
			top.ICEcoder.playSnake();
		},2000);		
	},
	...
	playSnake: function() {
		...
		top.ICEcoder.snakePreHistory = cM.getHistory();
		top.ICEcoder.snakePreContent = cM.getValue();
		top.ICEcoder.snakePreCursor = cM.getCursor();

		// Pick a random point for snake to come in and set head and 4 body parts off screen
		var randPos = Math.floor(Math.random()*50); 
		top.ICEcoder.snakePos = [
			[randPos,0],
			[randPos,-1],
			[randPos,-2],
			[randPos,-3],
		...
		top.ICEcoder.content.contentWindow.document.getElementById('game').style.display = 'block';
		top.ICEcoder.snakeDir = "down";
		top.ICEcoder.doSnake();

		// Every 0.1s, move snake
		top.ICEcoder.snakeInt = setInterval(function() {
			// Set new head X & Y pos according to direction
			var newHead = [];
			newHead[0] = top.ICEcoder.snakePos[0][0]+(top.ICEcoder.snakeDir == "right" ? 1 : top.ICEcoder.snakeDir == "left" ? -1 : 0);
			newHead[1] = top.ICEcoder.snakePos[0][1]+(top.ICEcoder.snakeDir == "down" ? 1 : top.ICEcoder.snakeDir == "up" ? -1 : 0);
			// Add new head and remove tail
			top.ICEcoder.snakePos.unshift(newHead);
			top.ICEcoder.snakePos.pop();
	...
	doSnake: function() {
		...
		top.ICEcoder.content.contentWindow.document.getElementById('game').innerHTML = "";
		// Start a new set of contents
		newInnerHTML = "";
		// For every part of snake, draw it's block in position
		for (var i=0; i<top.ICEcoder.snakePos.length; i++) {
			newInnerHTML += '<div style="position: absolute; diplay: inline-block; width: '+cW+'px; height: '+cH+'px; top: '+((top.ICEcoder.snakePos[i][1]*cH)+4)+'px; left: '+((top.ICEcoder.snakePos[i][0]*cW)+60)+'px; background: #fff"></div>';
		}
		// Set new content in game layer
		top.ICEcoder.content.contentWindow.document.getElementById('game').innerHTML = newInnerHTML;

		// Get line & ch value under snake head then line content
		lineData = cM.coordsChar({top: ((top.ICEcoder.snakePos[0][1]*cH)+4), left: ((top.ICEcoder.snakePos[0][0]*cW)+60)});
		lineContent = cM.getLine(lineData.line);

		// If not the last char on the line
		if (top.ICEcoder.snakePos[0][0]-1 <= lineContent.length-2) {
			spaceReplaceChars = "";
			...
			top.ICEcoder.snakePos.push([top.ICEcoder.snakePos[top.ICEcoder.snakePos.length-1][0],top.ICEcoder.snakePos[top.ICEcoder.snakePos.length-1][1]]);
			// Replace char under head with nothing if end of line, else with our replacement string
			cM.doc.replaceRange(top.ICEcoder.snakePos[0][0]-1 == lineContent.length-2 ? "" : spaceReplaceChars,lineData,{line: lineData.line, ch: lineData.ch+1});
			...
			if (top.ICEcoder.snakePos.length >= 5) {
				top.ICEcoder.snakePos.pop();
			}
		}
		// Detect if snake head has collided into itself
		collision = false;
		for (var i=1; i<top.ICEcoder.snakePos.length; i++) {
			if (top.ICEcoder.snakePos[i][0] == top.ICEcoder.snakePos[0][0] && top.ICEcoder.snakePos[i][1] == top.ICEcoder.snakePos[0][1]) {
				collision = true;
			}
		}
		...
		if (
			...
			top.ICEcoder.snakePos[0][0] < 0 || top.ICEcoder.snakePos[0][1] < 0 ||
			((top.ICEcoder.snakePos[0][0]*cW)+60) > scrollInfo.clientWidth || ((top.ICEcoder.snakePos[0][1]*cH)+4) > scrollInfo.clientHeight ||
			collision
			) {
			// Clear interval and hide game layer
			clearInterval(top.ICEcoder.snakeInt);
			top.ICEcoder.content.contentWindow.document.getElementById('game').style.display = 'none';
			// Set content, saved point, saved contents and history back to what they were pre game
			cM.setValue(top.ICEcoder.snakePreContent);
			top.ICEcoder.savedPoints[top.ICEcoder.selectedTab-1] = cM.changeGeneration(); 
			top.ICEcoder.savedContents[top.ICEcoder.selectedTab-1] = top.ICEcoder.snakePreContent;
			cM.setHistory(top.ICEcoder.snakePreHistory);
			...
			cM.setCursor(top.ICEcoder.snakePreCursor);
			cM.focus();
			// State we are no longer playing snake
			top.ICEcoder.snakePlaying = false;
		}
